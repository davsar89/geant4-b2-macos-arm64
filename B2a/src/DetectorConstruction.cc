...
DetectorConstruction::DetectorConstruction(const G4String& gdmlFile) : fGdmlFile(gdmlFile)
{
  fMessenger = new DetectorMessenger(this);

  fNbOfChambers = 5;
  // value-initialised: stays null if the geometry comes from GDML
...
{
  // Geometry from a GDML file (no schema validation: works offline)
  if (!fGdmlFile.empty()) {
...
void DetectorConstruction::ConstructSDandField()
{
  ...
  // of "Chamber_LV" (built-in geometry, or a GDML export of it).
  if (G4LogicalVolumeStore::GetInstance()->GetVolume("Chamber_LV", false)) {
    SetSensitiveDetector("Chamber_LV", trackerSD, true);
  }
  // GDML volumes tagged <auxiliary auxtype="SensDet" auxvalue="Tracker"/>
  for (const auto& [volume, auxList] : *fParser.GetAuxMap()) {
    for (const auto& aux : auxList) {
      if (aux.type == "SensDet" && volume->GetSensitiveDetector() == nullptr) {
        SetSensitiveDetector(volume, trackerSD);
      }
    }
  }
